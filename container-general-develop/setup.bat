...
rem setup.bat
rem
rem One-time setup to run before start.bat. Runs
rem scripts\setup.sh inside WSL Debian, which:
...
wsl.exe -d %DISTRO% -- bash -lc "cd \"$(wslpath -a '%SCRIPT_DIR%')\" && bash scripts/setup.sh"
if errorlevel 1 (
